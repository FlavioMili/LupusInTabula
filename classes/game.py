...
class Game:
    def __init__(self, chat_id, application):
    ...
    async def addPlayer(self, user_id, username):
        ...
        if len(self.players) < 10 and user_id not in self.players:
            ...
            if len(self.players) >= config.MIN_GIOCATORI and self.state == GameState.WAITING:
                self.startTimer(self.startGame, 60)
                print("Timer avviato, 60 secondi da ora")
            return True
        return False
    ...
    async def assignRoles(self):
        ...
        numVeggenti = 1 if num_players >= 4 else 0 # change to 5
        ...
        for uid, role in zip(players_ids, roles):
            ...
            try:
                await self.application.bot.send_message(uid, f"Sei un {role}")
            except Forbidden:
    ...
    async def dayPhase(self):
        ...
        for victim in self.wolf_kills:
            await self.eliminatePlayer(victim)
    ...
    async def endVotingPhase(self, context=None):
        ...
        if len(tied_players) > 1:
            ...
            if victim:
                ...
                await self.sendMessage(f"@{victim.username} e' stato eliminato per decisione del villaggio!")
    ...
    async def eliminatePlayer(self, player_id):
        ...
        if player_id in self.players:
            ...
            await self.application.bot.send_message(
                self.chat_id, f"@{self.players[player_id].username} e' stato eliminato dal gioco."
            )
        else:
